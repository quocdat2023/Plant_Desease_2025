...
# Khởi tạo memory lưu lịch sử hội thoại
memory = ConversationBufferMemory(memory_key="chat_history", return_messages=True)


def format_chat_history(memory):
    messages = memory.load_memory().get("chat_history")
    if not messages:
...
def query():
    ...
    # Lấy lịch sử chat để chèn vào prompt
    ...
    memory.save_context({"role": "user", "content": question}, {"role": "assistant", "content": answer})
...
def draft_judgment():
    ...
    # Lưu lịch sử hội thoại
    memory.save_context({"role": "user", "content": case_details}, {"role": "assistant", "content": judgment})
